...
### Requirement: 9-Layer Health Verification

`tokenwise doctor` MUST verify all 9 stack layers and report `OK`, `WARN`, or `FAIL` for each. The command MUST run on unconfigured systems (no preconditions).
...
Exit 0 when all 9 are OK. Exit 1 when any layer is FAIL.

#### Scenario: All layers healthy

- GIVEN all 9 stack components are installed and responding
- WHEN the user runs `tokenwise doctor`
- THEN output contains exactly 9 lines each with `[OK]` prefix and exit code is 0
...
- GIVEN layers 1–5 are healthy and layers 6–9 are unreachable
- WHEN the user runs `tokenwise doctor`
- THEN layers 1–5 show `[OK]`, layers 6–9 show `[FAIL]`, and exit code is 1

*Cargo tests*:
- `test::doctor::reports_exactly_nine_layers`
